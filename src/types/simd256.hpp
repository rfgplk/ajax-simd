...
template <is_simd_256_type T, is_flag_type F> class v256
{
  ...
  {
    if constexpr ( stdlib::same_as<T, f256> ) {
      value = _mm256_setzero_ps();
    }
    if constexpr ( stdlib::same_as<T, d256> ) {
      value = _mm256_setzero_pd();
    }
    if constexpr ( stdlib::same_as<T, i256> ) {
...
public:
  ...
  // end of ints
  v256(i8 _e0)
    requires stdlib::is_same_v<T, i256>
  {
  ...
  v256(float a)
    requires stdlib::is_same_v<T, f256>
  {
    value = _mm256_set1_ps(a);
  }
  v256(double a)
    requires stdlib::is_same_v<T, d256>
  {
  ...
  template <typename R>
    requires(stdlib::is_integral_v<R>)
  constexpr auto
  operator[](const R a)
  {
  ...
  operator+=(double x)
  {
    if constexpr ( stdlib::is_same_v<T, d256> ) {
    ...
    requires is_int_flag_type<A>
  {
    ...
    if constexpr ( stdlib::is_same_v<T, i256> ) {
      if constexpr ( __is_8_wide<F>() ) {
  ...
  operator/=(const v256 &o)
  {
    ...
    if constexpr ( stdlib::is_same_v<T, f256> ) {
      ...
      static_assert(!stdlib::is_same_v<T, i256>, "No SIMD integer division for __m256i (AVX2)");
    }
    return *this;
  }
  constexpr inline v256
